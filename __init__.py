from . import lib
from . import models
from . import controllers
from odoo.api import Environment, SUPERUSER_ID


def post_install_hook(cr, registry):
    """
    This will provide access of the module to the existing users at the time of installation.
    """

    env = Environment(cr, SUPERUSER_ID, {})
    ks_user_data = {"users": [(4, user_id.id) for user_id in (env['res.users'].search([]))]}
    ks_woo_access = env.ref('ks_woocommerce.ks_woocommerce_group')
    ks_woo_access.write(ks_user_data)


def uninstall_hook(cr, registry):
    ''' Need to reenable the `product` pricelist multi-company rule that were
        disabled to be 'overriden' for multi-website purpose
    '''
    env = Environment(cr, SUPERUSER_ID, {})
    #     for rec in env['ks.woo.product.tag'].search([]):
    #         rec.unlink()
    for rec in env['ks.woocommerce.instances'].search([]):
        if rec.ks_auto_update_stock:
            rec.ks_aus_cron_id.unlink()
        if rec.ks_auto_update_order_status:
            rec.ks_auos_cron_id.unlink()
        if rec.ks_auto_import_order:
            rec.ks_aio_cron_id.unlink()
        if rec.ks_auto_import_product:
            rec.ks_aip_cron_id.unlink()
        rec.unlink()
